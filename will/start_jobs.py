...
def submit(command, jobname, n_gpus, n_hours):
    ...
    sbatch_command = f"sbatch -p plai --export=all --ntasks=1 -t {n_hours}:0:0 --mem={12000*max(1, n_gpus)} --cpus-per-task {5*max(1, n_gpus)} --gres=gpu:{n_gpus} -J {jobname} --output=\"{output_dir}/%j.out\" --error=\"{output_dir}/%j.err\" job.sh"
